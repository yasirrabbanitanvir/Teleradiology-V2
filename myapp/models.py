...
class DICOMImage(models.Model):
    ...
    referral_file = models.FileField(upload_to='referrals/', null=True, blank=True)
...
class ReportTemplate(models.Model):
    BODY_PART_CHOICES = [
        ...
        ("MRI TEMPLATES", "MRI TEMPLATES"),
        ("X-RAY TEMPLATES", "X-RAY TEMPLATES"),
    ]
    MODALITY_CHOICES = [
        ("CT", "CT"),
        ("MR", "MR"),
    ...
    body_part = models.CharField("Body Part", max_length=100, choices=BODY_PART_CHOICES)
    modality = models.CharField("Modality", max_length=10, choices=MODALITY_CHOICES)
...
class ActiveViewer(models.Model):
    study_uid = models.CharField(max_length=64, db_index=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['study_uid', 'last_active']),
        ]
    
    def __str__(self):
        return f"{self.user_name} viewing {self.study_uid}"
    
    @classmethod
    def cleanup_stale_viewers(cls, timeout_minutes=30):
        from django.utils import timezone
        from datetime import timedelta
        cutoff_time = timezone.now() - timedelta(minutes=timeout_minutes)
        cls.objects.filter(last_active__lt=cutoff_time).delete()
    
    @classmethod
    def get_active_viewers(cls, study_uid):
        from django.utils import timezone
        from datetime import timedelta
